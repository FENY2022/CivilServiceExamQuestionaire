...
<body class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-100 font-sans text-slate-900">
    ...
    <main class="mx-auto grid min-h-screen w-full max-w-6xl place-items-center gap-7 px-4 py-8 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <section class="w-full rounded-[2rem] border border-blue-100 bg-white/95 p-7 shadow-2xl sm:p-9">
            ...
            <form method="post" class="mt-7 grid gap-5" id="registerForm">
    ...
    (function () {
        ...
        function submitDemoQuiz() {
            ...
            resultArea.innerHTML = `
                <p class="text-xs font-extrabold uppercase tracking-[.18em] text-brand-600">Your Sample Score</p>
                ...
                <p class="mt-3 font-semibold leading-7 text-slate-600">${percent >= 80 ? 'Nice score. You can practice more with the full reviewer.' : 'Good start. Register now and improve with more practice questions.'}</p>
                <div class="mt-5 rounded-3xl border border-yellow-200 bg-yellow-50 p-5">
                    <h4 class="text-2xl font-black text-brand-950">Want more questions?</h4>
                    <p class="mt-2 font-semibold leading-7 text-slate-700">Register now to access the full reviewer with more questionnaires, timer options, scoring, and explanations.</p>
                    <button class="mt-4 rounded-2xl bg-gradient-to-r from-brand-700 to-brand-600 px-5 py-3 font-black text-white shadow-xl transition hover:-translate-y-0.5" id="demoRegisterNow" type="button">Register Now</button>
                </div>
            `;
