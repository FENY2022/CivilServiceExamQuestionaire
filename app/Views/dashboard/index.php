...
<main class="mx-auto w-full max-w-6xl px-4 py-8">
    <section class="rounded-[2rem] border border-blue-100 bg-gradient-to-br from-white to-blue-50 p-5 shadow-2xl sm:p-8">
        ...
        <h1 class="mt-3 break-words text-2xl font-black text-brand-950 sm:text-3xl lg:text-4xl">Welcome, <?= esc($name) ?></h1>
    ...
    <section class="mt-7 grid gap-6 lg:grid-cols-2">
        <article class="relative overflow-hidden rounded-[2rem] border border-blue-100 bg-white p-5 shadow-2xl sm:p-8">
            ...
            <div class="relative mt-7 grid gap-3 sm:grid-cols-2">
                <a class="inline-flex w-full justify-center rounded-2xl bg-gradient-to-r from-brand-700 to-brand-600 px-5 py-4 text-center font-black text-white shadow-xl transition hover:-translate-y-0.5" href="<?= site_url('reviewer?type=professional&timer=1') ?>">Start with Timer</a>
                <a class="inline-flex w-full justify-center rounded-2xl border border-blue-200 bg-white px-5 py-4 text-center font-black text-brand-700 shadow-lg transition hover:-translate-y-0.5 hover:bg-blue-50" href="<?= site_url('reviewer?type=professional&timer=0') ?>">Start without Timer</a>
            </div>
        </article>
        <article class="relative overflow-hidden rounded-[2rem] border border-blue-100 bg-white p-5 shadow-2xl sm:p-8">
            ...
            <div class="relative mt-7 grid gap-3 sm:grid-cols-2">
                <a class="inline-flex w-full justify-center rounded-2xl bg-gradient-to-r from-brand-950 to-brand-900 px-5 py-4 text-center font-black text-white shadow-xl transition hover:-translate-y-0.5" href="<?= site_url('reviewer?type=subprofessional&timer=1') ?>">Start with Timer</a>
                <a class="inline-flex w-full justify-center rounded-2xl border border-blue-200 bg-white px-5 py-4 text-center font-black text-brand-700 shadow-lg transition hover:-translate-y-0.5 hover:bg-blue-50" href="<?= site_url('reviewer?type=subprofessional&timer=0') ?>">Start without Timer</a>
            </div>
        </article>
    </section>
...
<div class="fixed inset-0 z-50 grid place-items-center bg-slate-950/70 px-4 backdrop-blur-sm" id="guestNoticeModal">
    <div class="w-full max-w-lg rounded-[2rem] border border-blue-100 bg-white p-7 shadow-2xl sm:p-9">
        ...
        <div class="mt-5 grid gap-4 text-base font-semibold leading-7 text-slate-700"><p>Kini nga reviewer kay para lang sa review purposes ug libre ra. Walay personal nga impormasyon nga kolektahon gikan sa mga users.</p><p>This reviewer is for review purposes only and is completely free. No personal information is collected from users.</p></div>
...
(function () {
    ...
    function submitDemoQuiz() {
        ...
        resultArea.innerHTML = '<p class="text-xs font-extrabold uppercase tracking-[.18em] text-brand-600">Sample Quiz Score</p><h3 class="mt-2 text-3xl font-black text-brand-950">You scored ' + score + '/' + questions.length + '</h3><div class="mt-3 text-5xl font-black text-brand-700">' + percent + '%</div><p class="mt-3 font-semibold leading-7 text-slate-600">' + (percent >= 80 ? 'Nice work. You are ready to try the full reviewer.' : 'Good start. Keep practicing to improve your score.') + '</p><div class="mt-5 rounded-3xl border border-yellow-200 bg-yellow-50 p-5"><h4 class="text-2xl font-black text-brand-950">Want more practice?</h4><p class="mt-2 font-semibold leading-7 text-slate-700">You are already logged in. Start a full timed exam above or keep practicing here.</p></div>';
    ...
    function resetDemoQuiz() {
        questions.forEach(question => {
            question.querySelectorAll('.demo-choice').forEach(choice => choice.classList.remove(...correctClasses, ...incorrectClasses));
...
<script>
(function () {
    const key = 'guestNoticeAccepted';
    const modal = document.getElementById('guestNoticeModal');
    const closeButton = document.getElementById('guestNoticeClose');
    if (!modal || !closeButton) return;
    if (sessionStorage.getItem(key) === '1') { modal.remove(); return; }
    closeButton.addEventListener('click', () => { sessionStorage.setItem(key, '1'); modal.remove(); });
})();
</script>
<?php endif; ?>
<?= $this->endSection() ?>
